...
(function () {
    ...
    async function userDocHasAdminRole(uid) {
        try {
            ...
            if (snap.exists()) {
                ...
                if (Array.isArray(data.roles) && data.roles.map(r => String(r).toLowerCase()).includes('admin')) return true;
            }
        } catch (err) {
            console.warn('Failed reading users/{uid} for admin role:', err);
        }
        return false;
    }

    async function adminCollectionHasUser(uid) {
        try {
            ...
            console.warn('Failed reading admins/{uid}:', err);
        }
        return false;
    }

    async function adminSettingsIncludes(user) {
        try {
            const snap = await getDoc(doc(db, 'settings', 'admins'));
            if (snap.exists()) {
                const data = snap.data() || {};
                const uids = Array.isArray(data.uids) ? data.uids : [];
                const emails = Array.isArray(data.emails) ? data.emails.map(e => normalizeEmail(e)) : [];
                const emailNorm = normalizeEmail(user.email);
                return (uids.includes(user.uid) || emails.includes(emailNorm));
            }
        } catch (err) {
            console.warn('Failed reading settings/admins:', err);
        }
        return false;
    }
    ...
    async function isAdminUser(user) {
        ...
        if (await userDocHasAdminRole(user.uid)) return true;
        if (await adminCollectionHasUser(user.uid)) return true;
        if (await adminSettingsIncludes(user)) return true;
        return false;
    }
    ...
    function wireAuth() {
        auth.onAuthStateChanged(async (user) => {
            try {
                ...
                console.error('Admin gating error:', err);
                hideAdminBlocks();
            }
        });
    }
